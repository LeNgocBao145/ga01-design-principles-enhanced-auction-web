...
function generateNewShortImageFilename(originalUrl, publicPath, targetPath, type) {
  const tempFilename = path.basename(originalUrl);
  const tempPath = path.join(publicPath, originalUrl);
  ...
  return { tempPath, newPath, newUrl };
}
...
function moveUploadedFiles(tempUrls, type) {
  ...
  for (const tempUrl of tempUrls) {
    const { tempPath, newPath, newUrl } = generateNewShortImageFilename(tempUrl, publicPath, targetPath, type);
...
export async function deleteInvoice(invoiceId) {
  return db("invoices").where("id", invoiceId).del();
}
