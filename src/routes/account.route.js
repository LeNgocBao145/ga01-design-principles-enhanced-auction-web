...
router.post("/resend-forgot-password-otp", async (req, res) => {
  const { email } = req.body;
  const result = await authService.handleResendForgotPasswordOtp(email);
  if (!result.success) {
    return res.render("vwAccount/auth/verify-forgot-password-otp", {
      email,
      error_message: result.message,
...
router.post("/signin", async function (req, res) {
  ...
  if (!user.email_verified) {
    const otpData = otpService.createOtp("verify_email", user.id);

    mailService.sendSigninMail(email, user.fullname, otpData.otp);
...
router.post("/signup", async function (req, res) {
  ...
  const isCaptchaValid = await captchaService.verifyCaptcha(recaptchaResponse);
  if (!isCaptchaValid) {
    errors.captcha = "Captcha verification failed. Please try again.";
  }

  console.log("gia tri erros: ", errors);
  ...
  try {
    ...
    // Xử lý lỗi do Service ném ra (VD: Trùng email)
...
router.post("/resend-otp", async (req, res) => {
  ...
  const otpData = await otpService.createOtp("verify_email", user.id);

  await mailService.sendResendOtpMail(email, user.fullname, otpData.otp);
